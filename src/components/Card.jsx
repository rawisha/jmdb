...
const Card = ({ movieData, removeOnly }) => {
  ...
  let heartStatusClasses = classNames({
    fa: true,
    "fa-heart-o": !favStatus,
    favorite: true,
    "fa-heart": favStatus,
  });
  //om removeOnly finns ska det istället bli - tecken istället för hjärta i favorites
  if (removeOnly) {
    heartStatusClasses = classNames({
      fa: true,
        ...
        {movieData.tv_id ? (
          <div className="card-image-container">
            <Link
              ...
              state={{ tvID: movieData?.tv_id, tvInfo: movieData }}
            >
              <img
                src={imgsrc}
                alt={
                  "Movie & Tv show poster name" + movieData?.name ||
                  movieData?.title ||
                  movieData.original_name
                }
                className="card-image"
                onClick={() => favContext.handleModelClick(movieData)}
              />
            </Link>
            <i
              id="heart"
              onClick={() => handleFavClick(movieData)}
              className={heartStatusClasses}
              aria-hidden="true"
            ></i>
          </div>
        ) : (
          <div className="card-image-container">
            <Link
              ...
              state={{ movieID: movieData.imdb_id }}
            >
              <img
                src={imgsrc}
                alt={
